...
try {
    ...
    if ($choice -eq "1") {
        ...
        $newContent = $newContent -replace 'GetConnectionString\("SqlServerConnection"\)', 'GetConnectionString("DefaultConnection")'
        $newContent = $newContent -replace 'options\.UseSqlServer\(connectionString\)', 'options.UseMySql(connectionString, serverVersion)'
